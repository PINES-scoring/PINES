...
job_id = 1
to_score = unique(gsub(" ","",to_score,fixed = TRUE))
to_weigh = unique(gsub(" ","",to_weigh,fixed = TRUE))
if(length(which(to_score == "")) > 0){to_score = to_score[(-1) * which(to_score == "")]}
if(length(which(to_weigh == "")) > 0){to_weigh = to_weigh[(-1) * which(to_weigh == "")]}
to_score = grep(pattern="^rs[0-9]",x=to_score,value=T)
to_weigh = grep(pattern="^rs[0-9]",x=to_weigh,value=T)
if(!(to_tissue_type %in% c("neuro","liver","immune","heart"))){stop("Malformed tissue input specification")}
to_constant = as.numeric(to_constant)
if((!is.na(to_constant)) & (!(to_constant %in% 1:10))){stop("Manual weighting constant must be an integer between 1 and 10")}
gwas_variants = unique(to_weigh)
scoring_variants = unique(to_score)
if(length(scoring_variants) == 0){stop("Please provide a list of variants to score!")}
if((length(gwas_variants) == 0) & (is.na(to_constant))){unweighted_analysis = TRUE}
if((length(gwas_variants) > 0) | (!is.na(to_constant))){unweighted_analysis = FALSE}
if(length(gwas_variants) > 0){fine_mapping_region = unique(c(scoring_variants,gwas_variants))} 
if(length(gwas_variants) == 0){fine_mapping_region = unique(scoring_variants)}
load(paste(annotation_path,"data5.RData",sep=""))
load(paste(annotation_path,"data6.RData",sep=""))
load(paste(annotation_path,"data7.RData",sep=""))
load(paste(annotation_path,"data8.RData",sep=""))
load(paste(annotation_path,"data9.RData",sep=""))
enhancer_regions_tmp1 = as.vector(unlist(read.table(paste(annotation_path,"data10.txt",sep=""))))
...
load(paste(annotation_path,"data3.RData",sep=""))
CTCF_regions_liver = CTCF_regions[which(CTCF_regions[,4] == "Liver"),]
tss_data = read.table(paste(annotation_path,"mart_export.txt",sep=""),header=T,sep="\t")
annotate_SNP = cmpfun(function(SNP_ID)
{
	initial_query = system(paste("grep -F -m 1 '",SNP_ID,";' ",annotation_path,"data1.vcf",sep=""), intern=T)
	if(length(initial_query) != 1){return(0)}
       	else
	{
		parsed_initial_query = strsplit(strsplit(initial_query,split=";")[[1]],split=" ")[[1]]
		if(length(parsed_initial_query) != 3){stop("ERROR 2.0")}
	...
	db_query = paste(system(paste("tabix ",annotation_path,"data4.vcf.gz ",SNP_chr,":",SNP_pos-100,"-",SNP_pos+100," | grep \t",SNP_ID,"\t",sep=""), intern=T),collapse="")
	if(db_query == ""){return(0)}
        if(length(db_query) != 1){stop("ERROR!")}
	if(length(as.vector(unlist(gregexpr(text=db_query,pattern="rs")[[1]]))) != 1){return(0)}
        locfun_query = grep(strsplit(strsplit(db_query,split="\t")[[1]][8],split=";")[[1]],pattern="DBSNP=",value=T)
        if(length(locfun_query) > 0){locfun_annot = strsplit(locfun_query,split="DBSNP=")[[1]][2]}else{locfun_annot = "intergenic"}
	if(!(locfun_annot %in% c("intergenic","INT"))){return(0)}
	...
	haploreg_location = strsplit(db_query,split="\t")[[1]][2]
        ...
        if(length(chrom_index) > 0){if(length(which((fantom_enhancers[chrom_index,2] <= as.numeric(haploreg_location)) & (fantom_enhancers[chrom_index,3] >= as.numeric(haploreg_location)))) > 0){is_fantom5_enhancer = 1}}
	is_enhancer = 0
	chrom_index = which(enhancer_regions[,1] == as.numeric(haploreg_chr))
	if(length(chrom_index) > 0){if(length(which((enhancer_regions[chrom_index,2] <= as.numeric(haploreg_location)) & (enhancer_regions[chrom_index,3] >= as.numeric(haploreg_location)))) > 0){is_enhancer = 1}}
        is_CTCF = 0
        chrom_index = which(CTCF_regions_liver[,1] == paste("chr",haploreg_chr,sep=""))
        if(length(chrom_index) > 0){if(length(which((CTCF_regions_liver[chrom_index,2] <= as.numeric(haploreg_location)) & (CTCF_regions_liver[chrom_index,3] >= as.numeric(haploreg_location)))) > 0){is_CTCF = 1}}
        is_promoter = 0
        chrom_index = which(promoter_indexes[,1] == paste("chr",haploreg_chr,sep=""))
        if(length(chrom_index) > 0){if(length(which((promoter_indexes[chrom_index,2] <= as.numeric(haploreg_location)) & (promoter_indexes[chrom_index,3] >= as.numeric(haploreg_location)))) > 0){is_promoter = 1}}
	tss_min_distance = -1
        chrom_index = which(tss_data[,1] == as.numeric(haploreg_chr))
	if(length(chrom_index) > 0){tss_min_distance = min(abs(tss_data[chrom_index,2] - as.numeric(haploreg_location)))}
	...
	if(length(honey_dhs_query) > 0){is_honey_dhs = 1}else{is_honey_dhs = 0}
        maf_query = grep(strsplit(strsplit(db_query,split="\t")[[1]][8],split=";")[[1]],pattern="AF=",value=T)
	if(length(maf_query) > 0){maf_annot = as.numeric(strsplit(strsplit(maf_query,split="AF=")[[1]][2],split=",")[[1]])}else{maf_annot = c(0,0,0,0)}
	...
	if(length(which((chrom_interact$InteractorAStart <= as.numeric(haploreg_location)) & (chrom_interact$InteractorAEnd >= as.numeric(haploreg_location)) & (chrom_interact$InteractorAChr == paste("chr",haploreg_chr,sep="")))) > 0){c4_annot = 1}else{if(length(which((chrom_interact$InteractorBStart <= as.numeric(haploreg_location)) & (chrom_interact$InteractorBEnd >= as.numeric(haploreg_location)) & (chrom_interact$InteractorBChr == paste("chr",haploreg_chr,sep="")))) > 0){c4_annot = 1}else{c4_annot = 0}}
	...
	if(length(histone_query) > 0)
	{
		...
		for(i in 1:nrow(histone_annot))
		{
			...
			if(is.na(row_index) | is.na(col_index)){stop("ERROR!")}
	...
	if(length(hmm15_query) > 0)
	{
		hmm15_annot = matrix(unlist(strsplit(strsplit(strsplit(hmm15_query,split="=")[[1]][2],split=",")[[1]],split="\\|")),ncol=2,byrow=T)
			...
			row_index = match(hmm15_annot[i,1],annotation_matrix[,1])
                        if(is.na(row_index)){stop("ERROR!")}
			if(strsplit(hmm15_annot[i,2],split="_")[[1]][1] %in% c(1,2,3,4,5,6,7,8)){annotation_matrix[row_index,5] = 1}else{annotation_matrix[row_index,5] = -1}
        	}
	}
	...
	if(length(hmm25_query) > 0)
	{
		hmm25_annot = matrix(unlist(strsplit(strsplit(strsplit(hmm25_query,split="=")[[1]][2],split=",")[[1]],split="\\|")),ncol=2,byrow=T)
			...
			row_index = match(hmm25_annot[i,1],annotation_matrix[,1])
                        if(is.na(row_index)){stop("ERROR!")}
                        if(strsplit(hmm25_annot[i,2],split="_")[[1]][1] %in% 1:20){annotation_matrix[row_index,6] = 1}else{annotation_matrix[row_index,6] = -1}
                }
	}
	...
	annotation_vector = c(annotation_vector,maf_annot,gerp_annot,siphy_annot,c4_annot,is_honey_dhs,is_enhancer,is_CTCF,tss_min_distance,SNP_ID,locfun_annot,is_promoter,is_fantom5_enhancer)
	names(annotation_vector)[(old_length + 1):length(annotation_vector)] = c("maf_AFR","maf_AMR","maf_ASN","maf_EUR","gerp_conservation","siphy_conservation","interactions_4C","HoneyBadger_DHS","is_enhancer","is_CTCF","tss_min_distance","SNP_ID","locfun_annot","is_promoter","is_fantom5_enhancer")
	annotation_vector = c(SNP_chr,SNP_pos,annotation_vector)
	names(annotation_vector)[1:2] = c("chrom","location")
	if(length(annotation_vector) != 906){stop("ERROR!")}
	write.table(t(annotation_vector),file=paste("annotation_vector_",job_id,"_",SNP_ID,".txt",sep=""),sep=",",row.names=F,quote=F)
	return(0)		
})
invisible(mclapply(X=fine_mapping_region,FUN=annotate_SNP,mc.cores=min(length(fine_mapping_region),ncores)))
system(paste("cat annotation_vector_",job_id,"_* > combined_annotations_",job_id,".txt",sep=""),ignore.stderr=T)
if(file.size(paste("combined_annotations_",job_id,".txt",sep="")) == 0){stop("The provided variants could not be annotated. Make sure that you are only attempting to score noncoding (intronic or intergenic) variants!")}
new_annotations = read.table(file=paste("combined_annotations_",job_id,".txt",sep=""),header=T,sep=",")
...
cell_types = read.table(paste(annotation_path,"cell_types.csv",sep=""),header=T,sep=",")
tss_data = read.table(paste(annotation_path,"mart_export.txt",sep=""),header=T,sep="\t")
if(!all(colnames(combined_annotations) == colnames(data))){stop("Error!")}
...
if(length(index_of_na) > 0){test_variant_indexes = test_variant_indexes[(-1)*index_of_na]}
if(length(test_variant_indexes) == 0){stop("The provided variants could not be annotated. Make sure that you are only attempting to score noncoding (intronic or intergenic) SNPs!")}
rm(combined_annotations)
invisible(gc())
pathogenic_variants = as.vector(unlist(read.table(paste(annotation_path,"data11.txt",sep=""))))
...
if(length(remove_all_indexes) > 0){background_indexes = background_indexes[(-1) * remove_all_indexes]}
...
if((length(intronic_background_indexes) < 157856) | (length(intergenic_background_indexes) < 284789)){stop("error!")}
...
if(length(zero_indexes) > 0){annotation_matrix = annotation_matrix[,(-1)*zero_indexes]}
invisible(gc())
if(length(gwas_variants) > 0)
{
	num_draws_enrich = 1000
	...
	for(i in 1:ncol(annotation_matrix))
	{
		...
		for(k in 1:ncol(matrix_of_matched_indexes)){null_dist[k] = sum(annotation_matrix[matrix_of_matched_indexes[,k],i])}
...
heart_cell_names = as.vector(unlist(cell_types[grep(x=cell_types[,2],pattern="Heart"),1]))
immune_cells = NULL
hdl_cells = NULL
neuro_cells = NULL
heart_cells = NULL
for(i in immune_cells_names){immune_cells = c(immune_cells,grep(pattern=i,x=colnames(annotation_matrix)))}
for(i in hdl_cells_names){hdl_cells = c(hdl_cells,grep(pattern=i,x=colnames(annotation_matrix)))}
for(i in neuro_cell_names){neuro_cells = c(neuro_cells,grep(pattern=i,x=colnames(annotation_matrix)))}
for(i in heart_cell_names){heart_cells = c(heart_cells,grep(pattern=i,x=colnames(annotation_matrix)))}
...
for(i in 1:30){smoothed_cov_intronic = smoothed_cov_intronic + evalues_intronic[i] * t(t(evectors_intronic[,i])) %*% t(evectors_intronic[,i])}
inverse_smoothed_cov_intronic = 0
for(i in 1:30){inverse_smoothed_cov_intronic = inverse_smoothed_cov_intronic + (1 / evalues_intronic[i]) * t(t(evectors_intronic[,i])) %*% t(evectors_intronic[,i])}
smoothed_cov_intergenic = 0
for(i in 1:30){smoothed_cov_intergenic = smoothed_cov_intergenic + evalues_intergenic[i] * t(t(evectors_intergenic[,i])) %*% t(evectors_intergenic[,i])}
inverse_smoothed_cov_intergenic = 0
for(i in 1:30){inverse_smoothed_cov_intergenic = inverse_smoothed_cov_intergenic + (1 / evalues_intergenic[i]) * t(t(evectors_intergenic[,i])) %*% t(evectors_intergenic[,i])}
invisible(gc())
if(length(gwas_variants) > 0){w = diag(feature_enrichment_pvalues)} 
if((length(gwas_variants) == 0) & (!is.na(to_constant)))
{
	w = diag(ncol(annotation_matrix))
	if(to_tissue_type == "neuro"){for(i in neuro_cells){w[i,i] = to_constant}}
        if(to_tissue_type == "liver"){for(i in hdl_cells){w[i,i] = to_constant}}
        if(to_tissue_type == "immune"){for(i in immune_cells){w[i,i] = to_constant}}
        if(to_tissue_type == "heart"){for(i in heart_cells){w[i,i] = to_constant}}
	for(i in TSS_indexes){w[i,i] = 1}
	for(i in DHS_indexes){w[i,i] = 1}
	for(i in conservation_indexes){w[i,i] = 1}
	for(i in interaction_4C_indexes){w[i,i] = 1}
}
if((length(gwas_variants) == 0) & (is.na(to_constant))){w = diag(ncol(annotation_matrix))}
...
get_pvalues = cmpfun(function(i)
{
	...
	cat(SNP_name,pvalue_method_unweighted,pvalue_method_weighted,"\n",file=paste("pvalues_",job_id,"_",SNP_name,".txt",sep=""))
        return(0)
})
rm(data)
invisible(gc())
background_distances_intronic_uw = as.vector(unlist(mclapply(X=intronic_background_indexes[1:150000],FUN=get_mah_distance_uw,type="INT",mc.cores=ncores)))
background_distances_intergenic_uw = as.vector(unlist(mclapply(X=intergenic_background_indexes[1:150000],FUN=get_mah_distance_uw,type="intergenic",mc.cores=ncores)))
background_distances_intronic_w = as.vector(unlist(mclapply(X=intronic_background_indexes[1:150000],FUN=get_mah_distance_w,type="INT",mc.cores=ncores)))
background_distances_intergenic_w = as.vector(unlist(mclapply(X=intergenic_background_indexes[1:150000],FUN=get_mah_distance_w,type="intergenic",mc.cores=ncores)))
invisible(gc())
mclapply(X=test_variant_indexes,FUN=get_pvalues,mc.cores=min(length(test_variant_indexes),ncores))
system(paste("rm -f annotation_vector_",job_id,"_*",sep=""))
system(paste("rm -f combined_annotations_",job_id,".txt",sep=""))
cat("SNP_ID Unweighted_PINES Weighted_PINES\n",file=file_for_output)
system(paste("cat pvalues_",job_id,"_* >> ",file_for_output,sep=""))
system(paste("rm -f pvalues_",job_id,"_*",sep=""))
